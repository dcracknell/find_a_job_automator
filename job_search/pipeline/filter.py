...
def _build_title_exclude_pattern(title_excludes: list[str]) -> re.Pattern | None:
        ...
        if not title_excludes:
                    return None
                # Sort longest-first to avoid prefix-shadowing issues, then escape
...
def apply_filters(
    ...
    # Build the title-exclusion pattern once, outside the loop
    ...
    for rec in records:
        ...
        # Title relevance — drop only titles matching profile title_excludes
