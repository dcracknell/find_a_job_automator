"""Tests for job_search.pipeline.filter — title relevance, distance, locations.
...
- On-site jobs beyond search_radius_miles are dropped even when remote_ok=true;
  remote jobs are dropped when remote_ok=false.
"""
...
def _mock_conn():
    """Return a mock SQLite connection with no companies in cooldown."""
    conn = MagicMock()
...
class TestApplyFilters:
    def _run(self, records, title_excludes=None, profile=None):
        if profile is None:
            profile = _make_profile(
                title_excludes=title_excludes or _HARDWARE_PROFILE_TITLE_EXCLUDES
            )
        conn = _mock_conn()
    ...
    def test_salary_filter_still_works(self):
        ...
        result = self._run(records, profile=profile)
        passed_titles = {r.title for r in result}
...
# ---------------------------------------------------------------------------
# Remote / distance / location semantics
# ---------------------------------------------------------------------------


class TestLocationFiltering:
    # Sheffield home is set in _make_profile: (53.38, -1.47)
    _LONDON = (51.5074, -0.1278)        # ~140 miles from Sheffield
    _MANCHESTER = (53.4808, -2.2426)    # ~33 miles from Sheffield

    def _run(self, records, profile):
        return apply_filters(records, profile, _mock_conn(), today=date(2026, 6, 30))

    def test_onsite_beyond_radius_dropped_even_when_remote_ok(self):
        """remote_ok=true must not disable the radius check for on-site jobs."""
        profile = _make_profile(title_excludes=[])
        rec = _make_record(
            "Graduate Engineer", location="London",
            lat=self._LONDON[0], lon=self._LONDON[1],
        )
        assert self._run([rec], profile) == []

    def test_onsite_within_radius_kept(self):
        profile = _make_profile(title_excludes=[])
        rec = _make_record(
            "Graduate Engineer", location="Manchester",
            lat=self._MANCHESTER[0], lon=self._MANCHESTER[1],
        )
        assert len(self._run([rec], profile)) == 1

    def test_onsite_without_coords_gets_benefit_of_doubt(self):
        profile = _make_profile(title_excludes=[])
        rec = _make_record("Graduate Engineer", location="Somewhere", lat=None, lon=None)
        assert len(self._run([rec], profile)) == 1

    def test_remote_job_dropped_when_remote_not_ok(self):
        profile = _make_profile(title_excludes=[])
        profile["remote_ok"] = False
        rec = _make_record("Graduate Engineer", location="Remote")
        assert self._run([rec], profile) == []

    def test_remote_job_kept_when_remote_ok(self):
        profile = _make_profile(title_excludes=[])
        rec = _make_record("Graduate Engineer", location="Fully remote (UK)")
        assert len(self._run([rec], profile)) == 1

    def test_location_excludes_drop_foreign_listings(self):
        profile = _make_profile(title_excludes=[])
        profile["filters"]["location_excludes"] = ["United States", "California"]
        records = [
            _make_record("Graduate Engineer", location="San Jose, California"),
            _make_record("Graduate Engineer", location="Sheffield, UK"),
        ]
